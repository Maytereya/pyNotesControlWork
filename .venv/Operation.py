...
def add():
    note = Constructor.create_note(min_size)
    array = Handler.read_file()
    for notes in array:
        if Note.Note.get_id(note) == Note.Note.get_id(notes):
            Note.Note.set_id(note)
    array.append(note)
    Handler.write_file(array, 'a')
...
def id_rewrite():
    id = input(
        'ID  заметки, которую Вы хотите перезаписать: ')
    array = Handler.read_file()
    logic = True
    for notes in array:
    ...
    if logic:
        print('Заметка не найдена. Проверьте правильность ID!')
    Handler.write_file(array, 'a')
...
def date_show():
    date = input('Введите дату и время редактирования: ')
    array = Handler.read_file()
    logic = True
    for notes in array:
        if date == Note.Note.get_date(notes):
            ...
            print(Note.Note.info(notes))
    if logic == True:
        print('Заметка не найдена. Правильный ли ID?')
    Handler.write_file(array, 'a')
